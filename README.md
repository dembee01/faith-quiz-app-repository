...
For debug APKs to update an existing installation across different machines,
every machine must use the same signing key. Configure the shared release
keystore below; debug builds automatically use it when configured. Without it,
Android's machine-local debug keys are intentionally different and an update
requires uninstalling the old app.
...
Increment the version for each update (Android will reject a lower version):

```bash
./gradlew assembleDebug -PVERSION_CODE=2 -PVERSION_NAME=1.1
```
